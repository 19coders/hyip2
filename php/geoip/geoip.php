...
function ip_code($ip = FALSE){
  $geoip = geo_ip::getInstance(dirname(__FILE__) . "/geoip.dat");  
  $ip = $ip == FALSE ? $_SERVER['REMOTE_ADDR'] : $ip;
  return $geoip->lookupCountryCode($ip);
}

function ip_name($ip = FALSE){
  $geoip = geo_ip::getInstance(dirname(__FILE__) . "/geoip.dat");  
  $ip = $ip == FALSE ? $_SERVER['REMOTE_ADDR'] : $ip;
  return $geoip->lookupCountryName($ip);
}

function ip_img_style($ip = FALSE){
